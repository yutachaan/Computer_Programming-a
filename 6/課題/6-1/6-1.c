...
void BM(int filesize, char *text, char *pattern) {
  ...
  // スキップテーブルの作成
  for (int c = 0; c < UCHAR_MAX; c++) skip[c] = pattern_len; // パターン内に存在しない場合
  for (int i = 0; i < pattern_len; i++) skip[pattern[i]] = pattern_len - i - 1; // パターン内に存在する場合(pattern内に同じ文字が複数存在する場合は後ろ側の位置に合わせる)
